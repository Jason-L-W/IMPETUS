...
class TrackPart:
    ...
    def loopCG_func(h):
        ngs = 4
        g = 9.8
        # Use a baseline initial height H to generate the characteristic shape
        H_initial = h * 1.5  
        dt = 0.01

        # Initialize conditions
        X, Y, S, V = [0], [0], [0], [np.sqrt(2 * g * H_initial)]
        Nx, Ny, Nz, theta = [0], [1], [0], [0]
        R = [V[0]**2 / (g * (ngs - np.cos(theta[0])))] 
        ...
        # Find the actual peak height generated by the physics loop
        actual_peak_height = np.max(Y)
        
        # Calculate the exact correction factor needed
        scale_factor = h / actual_peak_height
        
        # Scale the structural dimensions smoothly
        X = X * scale_factor
        Y = Y * scale_factor
        S = S * scale_factor  # Scaling arc length keeps velocity ratios intact
    ...
    def initialize_track_data(parts):
        ...
        for part in parts:
            ...
            # For Launcher sections, set a default acceleration value
            if section_type == "Launcher":
                track_data[section_name]["stats"]["acceleration"] = 1.5
            # For brake sections, set a default acceleration value
            if section_type == "Brake":
                track_data[section_name]["stats"]["acceleration"] = -1.5
    ...
    def combine_tracks(*parts, section_folder ,coaster_name):
        ...
        for part in parts[1:]:
            ...
            # For corkscrew sections, the bottom radius is is the same as the top radius since it is a constant radius
            if section_type == "Corkscrew":
                track_data[section_name]["stats"]["r_bottom"] = r_top
...
if __name__ == "__main__":
    ...
    ax.plot(X, Y, Z, 'b-', label='Track Path')
    ax.plot(X + Nx, Y + Ny, Z + Nz, 'r-', alpha=0.4, label='Normals')
